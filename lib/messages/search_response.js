 ...
 *                  Defaults to 'false'.
 * @param {Object} options see: DN.format for details
 */
SearchResponse.prototype.send = function (entry, nofiltering, options) {
  ...
  if (entry instanceof SearchEntry || entry instanceof SearchReference) {
    ...
    var save = entry;
    var _dn = typeof (save.dn) === 'string' ? parseDN(save.dn) : save.dn;
    _dn.setFormat(options);
